...
const nimScopeStore = new Map();
nimScopeStore.set("gateway:config", JSON.stringify({
  routing: { failover: true, load_balance: true },
  settings: { model_cache_ttl: 3600, request_timeout_ms: 30000, upstream_cooldown_ttl: 60 },
  upstreams: [
...
const nimScopeEnv = {
  ...
  ...env,
  KV: {
    async get(key, type) { const value = nimScopeStore.get(key); return type === "json" && value ? JSON.parse(value) : value || null; },
    async put(key, value) { nimScopeStore.set(key, value); },
    async delete(key) { nimScopeStore.delete(key); },
  },
  CLIENTS_JSON: JSON.stringify([{ name: "nim-scope-client", key: "sk-nim-scope", models: ["*"], upstreams: ["nim-scope-active", "nim-scope-idle", "scope-other"] }]),
};
const nimScopeWarm = await worker.default.fetch(new Request("https://gw.test/v1/chat/completions", {
  method: "POST", headers: { authorization: "Bearer sk-nim-scope", "content-type": "application/json" }, body: JSON.stringify({ model: "nim-scope-warm", messages: [] }),
}), nimScopeEnv);
const nimScopedChat = await worker.default.fetch(new Request("https://gw.test/v1/chat/completions", {
  method: "POST", headers: { authorization: "Bearer sk-nim-scope", "content-type": "application/json" }, body: JSON.stringify({ model: "nvidia-nim/nim-scope-model", messages: [] }),
}), nimScopeEnv);
assert.equal(nimScopedChat.headers.get("x-llm-gateway-upstream"), "nim-scope-idle");
...
assert.equal(nimScopedResponses.headers.get("x-llm-gateway-upstream"), "nim-scope-idle");
const nimScopedCompact = await worker.default.fetch(new Request("https://gw.test/v1/responses/compact", {
  method: "POST", headers: { authorization: "Bearer sk-nim-scope", "content-type": "application/json" }, body: JSON.stringify({ model: "nvidia-nim/nim-scope-model", input: "Summarize this." }),
}), nimScopeEnv);
assert.equal(nimScopedCompact.headers.get("x-llm-gateway-upstream"), "nim-scope-idle");
const nimScopedAnthropic = await worker.default.fetch(new Request("https://gw.test/v1/messages", {
  method: "POST", headers: { "x-api-key": "sk-nim-scope", "content-type": "application/json", "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: "nvidia-nim/nim-scope-model", max_tokens: 8, messages: [{ role: "user", content: "hi" }] }),
}), nimScopeEnv);
assert.equal(nimScopedAnthropic.headers.get("x-llm-gateway-upstream"), "nim-scope-idle");
await nimScopeWarm.text();
