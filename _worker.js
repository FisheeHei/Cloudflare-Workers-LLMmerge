        ...
        .map((item, index) => {
          ...
          return '<article class="upstream-card" data-id="' + esc(item.id) + '">' +
            '<div class="upstream-head">' +
              '<strong>' + esc(item.note || item.name || "未命名上游") + '</strong>' +
            ...
            '<div class="row">' +
              ...
              '<div class="field span-4"><label>路径白名单</label><input data-field="paths" value="' + esc((item.paths || []).join(", ")) + '" placeholder="/v1/chat/completions,/v1/embeddings"></div>' +
    ...
    function renderClients() {
      ...
      host.querySelectorAll("button[data-client-id]").forEach((button) => {
        button.addEventListener("click", async () => {
          await deleteClient(button.dataset.clientId);
        });
      });
    }
